...
- 모바일에서 가로 스크린 문제는 가로 스크롤은 어떤 컨테이너가 어떤 요소들을 담고 있을 때 자식 요소들의 너비가 컨테이너보다 넓을 때 생긴다.
- 아이폰에서 스크롤에 따라 상하단에 생기는 바의 크기도 고려해서 resize를 결정할 값을 정해야 한다. 적당히 900으로 잡자. 그리고 핸드폰을 세로로 보다 가로로 보게 되면 orientationchange라는 이벤트가 발생하는데 돌릴 때 바로 이 이벤트가 발생하나 방향에 따라 화면이 set 되기 까지는 시간이 걸린다. setTimeout으로 0.5초 정도 걸어주자.

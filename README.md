...
- 중간 부분에서 새로고침 했을 때 이상하게 나온다. 스크롤을 디테일하게 조작하여 만드는 페이지는 새로고침 시 강제로 위로 보내는 경우가 많다. 하지만 여기서는 그러지 않는다. 애플도 그러지 않으니깐. 그럼 왜 이 문제가 발생할까?
- 그건 바로 지금 스크롤을 해야지 위치를 구해서 보여줄 걸 보여주고 하는데 새로고침 하면 스크롤을 하지 않은 상태로 위치를 모르기 때문이다. 또한 지금 heightNum 값으로 height 값을 동적으로 주고 있는데 처음 로드될 때는 heightNum이 적용되지 않은 상태, 그냥 화면 높이만큼 높이가 결정되기 때문에 새로고침 시 높이가 문제가 생긴다.
- 0, 2, 3 섹션은 나중에 스크립트로 계산되는 높이를 사용해야 하나 1 섹션 (보통 스크롤 섹션)은 처음부터 가진 높이를 바로 쓰는데 처음엔 모든 섹션이 처음부터 가진 높이로 인지되기 때문에 발생하는 문제다.
- debugger를 사용 break을 걸고 자바스크립트 하나 하나 실행해서 확인해보자.
- 그럼 이 초기 높이 설정에 의한 문제는 어떻게 해야할까? 단순히 생각해서 로드가 끝나기 전에는 높이를 없앤디 끝난 후에 계산된 높이를, 그리고 기본 높이만 가진 섹션이면 기본 높이를 주고 보이게 하자.
- 이번에는 그럼 새로고침 했을 때 이미지가 보이지 않고 스크롤을 하는 순간 이미지가 보이게 할 수도 있을 것이다. 다양한 방법들이 있을 것이다. 로드 됐을 때 스크롤 위치를 구한다든가... 근데 그냥 간단하게 스크롤이 필요한 것 뿐이다. 그럼 그냥 살짝 스크롤을 하자.
